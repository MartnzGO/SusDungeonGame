...
const GENERATOR_SCRIPT = preload("res://addons/Map Generator Testing Ground/generator.gd")
const EXIT_SCRIPT = preload("res://addons/Map Generator Testing Ground/modular_room_exit.gd")
const OBJECT_SCRIPT = preload("res://addons/Map Generator Testing Ground/intersect_obb.gd")
	...
	for shape1 in my_space.get_children():
		...
		for shape2 in room_space.get_children():
			if !(shape2 is CollisionShape) or !(shape2.shape is BoxShape):
				continue
			if preload("res://addons/Map Generator Testing Ground/intersect_obb.gd").intersect(shape1, shape2):
				return true
	return false
